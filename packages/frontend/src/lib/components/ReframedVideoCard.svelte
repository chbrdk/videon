<script lang="ts">
  ...
  $: videoUrl =
    reframedVideo.status === 'COMPLETED' && reframedVideo.outputPath
...
<div class="reframed-card">
  ...
  <div class="video-container {aspectRatioClass} relative overflow-hidden bg-black rounded-t-lg">
    ...
    {:else if isCompleted && thumbnailUrl}
      ...
      <img src={thumbnailUrl} alt="Thumbnail" class="absolute inset-0 w-full h-full object-cover" />
      ...
      >
        <div class="play-icon opacity-80 hover:opacity-100 transition-opacity">
          <MaterialSymbol icon="play_arrow" fontSize={80} />
        </div>
      </button>
      ...
      <div class="absolute bottom-4 right-4">
        <div class="flex flex-col items-end gap-2">
          <!-- Status Chip -->
          <div
            class="status-chip-mini"
            class:bg-yellow-500={reframedVideo.status === 'PROCESSING'}
            class:bg-green-500={reframedVideo.status === 'COMPLETED'}
            class:bg-red-500={reframedVideo.status === 'ERROR'}
          >
          ...
          <div class="info-chip">
            <span class="w-4 h-4 flex items-center justify-center"
              ><MaterialSymbol icon="aspect_ratio" fontSize={16} /></span
            >
            <span>{reframedVideo.aspectRatio}</span>
          </div>
          <div class="info-chip">
            <span class="w-4 h-4 flex items-center justify-center"
              ><MaterialSymbol icon="access_time" fontSize={16} /></span
            >
            <span>{formatDuration(reframedVideo.duration)}</span>
          </div>
          <div class="info-chip">
            <span class="w-4 h-4 flex items-center justify-center"
              ><MaterialSymbol icon="storage" fontSize={16} /></span
            >
    ...
    {:else if isCompleted}
      ...
      <div
        class="absolute inset-0 bg-gradient-to-br from-blue-500/20 to-purple-500/20 flex items-center justify-center"
      >
        <button
          on:click={handlePlay}
        ...
        >
          <div
            class="play-icon w-16 h-16 opacity-60 hover:opacity-80 transition-opacity flex items-center justify-center"
          >
      ...
      <!-- Info -->
      <div
        class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-3 text-white text-xs"
      >
      ...
      >
        ...
        <div class="absolute bottom-0 left-0 right-0 h-1 bg-white/10">
          <div
            ...
            style="width: {reframedVideo.progress * 100}%"
          ></div>
        </div>
      </div>
    {/if}
