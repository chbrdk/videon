<script lang="ts">
  ...
  function getBorderColor(): string {
    if (isHovered && hoverable) {
      return `rgba(${MSQDX_COLORS.brand.green.replace('#', '').match(/.{2}/g)?.map(x => parseInt(x, 16)).join(', ')}, 0.4)`;
    }
...
<style>
  ...
  @media (min-width: 768px) {
    .msqdx-glass-card:not([style*="border-radius"]):not(.no-padding):not([style*="--border-radius: 40px"]):not([style*="--border-radius:40px"]) {
