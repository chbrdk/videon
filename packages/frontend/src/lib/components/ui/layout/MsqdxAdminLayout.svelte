<script lang="ts">
  ...
  import { MSQDX_COLORS, MSQDX_SPACING, MSQDX_RESPONSIVE, MSQDX_TYPOGRAPHY } from '$lib/design-tokens';
...
<div
  ...
  style="
    ...
    height: {isMobile ? `calc(100vh - ${MSQDX_SPACING.scale.xxs * 2}px)` : `calc(100vh - ${MSQDX_SPACING.scale.xs * 2}px)`};
    overflow: hidden;
  "
>
  <!-- Sidebar -->
  ...
  <div
    ...
    style="
      ...
      min-height: 100vh;
      position: relative;
      border: {isMobile ? `${MSQDX_RESPONSIVE.outerShell.border.xs}px solid var(--msqdx-color-brand-orange)` : `${MSQDX_RESPONSIVE.outerShell.border.md}px solid var(--msqdx-color-brand-orange)`};
      border-top: {isMobile ? `${MSQDX_RESPONSIVE.outerShell.border.xs}px solid var(--msqdx-color-brand-orange)` : `${MSQDX_RESPONSIVE.outerShell.border.md}px solid var(--msqdx-color-brand-orange)`};
      border-right: {isMobile ? `${MSQDX_RESPONSIVE.outerShell.border.xs}px solid var(--msqdx-color-brand-orange)` : `${MSQDX_RESPONSIVE.outerShell.border.md}px solid var(--msqdx-color-brand-orange)`};
      border-bottom: {isMobile ? `${MSQDX_RESPONSIVE.outerShell.border.xs}px solid var(--msqdx-color-brand-orange)` : `${MSQDX_RESPONSIVE.outerShell.border.md}px solid var(--msqdx-color-brand-orange)`};
      border-left: {isMobile ? `${MSQDX_RESPONSIVE.outerShell.border.xs}px solid var(--msqdx-color-brand-orange)` : '0'};
      border-radius: {MSQDX_RESPONSIVE.outerShell.radius}px;
      background-color: {currentTheme === 'dark' ? MSQDX_COLORS.dark.background : MSQDX_COLORS.light.background};
    ...
    >
      ...
      <div
        style="
          ...
          border-radius: {isMobile ? `${MSQDX_SPACING.borderRadius.sm}px` : `${MSQDX_SPACING.borderRadius.md}px`};
          height: fit-content;
        "
      >
        <div
        ...
        <div
          style="
            height: 24px;
            border-left: 1px solid {currentTheme === 'dark' ? MSQDX_COLORS.brand.black : MSQDX_COLORS.light.textPrimary};
        ...
        {#if pageTitle}
          <h1
            style="
              ...
              margin-top: -15px;
              letter-spacing: -2px;
              color: {currentTheme === 'dark' ? MSQDX_COLORS.dark.textPrimary : MSQDX_COLORS.light.textPrimary};
              display: {isMobile ? 'none' : 'block'};
              font-weight: 800;
              margin: 0;
            "
      ...
      {#if isMobile}
        <button
          ...
          style="
            ...
            top: 7px;
            z-index: 1201;
            color: {currentTheme === 'dark' ? MSQDX_COLORS.brand.black : MSQDX_COLORS.light.textPrimary};
            padding: 16px;
      ...
      {#if isMobile && !drawerOpen}
        <button
          ...
          "
        >
          <svg width="24" height="24" viewBox="0 0 24 24" fill={currentTheme === 'dark' ? MSQDX_COLORS.brand.white : MSQDX_COLORS.brand.black}>
    ...
    >
      {#if title}
        <div style="margin-bottom: 1.5rem;">
          <h1 style="font-size: {MSQDX_TYPOGRAPHY.fontSize['2xl']}; margin-bottom: 0.5rem; font-weight: 600;">
            {title}
          </h1>
          {#if subtitle}
            <p style="color: {currentTheme === 'dark' ? MSQDX_COLORS.dark.textSecondary : MSQDX_COLORS.light.textSecondary};">
              {subtitle}
            </p>
          {/if}
        </div>
