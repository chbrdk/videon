<script lang="ts">
  ...
  function handleSegmentContextMenu(
    event: CustomEvent<{ event: MouseEvent; segment: VoiceSegment }>
  ) {
  ...
  $: console.log(
    ...
    containerWidth,
    'calculation:',
    timelineDuration * $zoomLevel * 20
  );

  // Debug: Track audioStemClips changes
  $: console.log(
    '🎵 Timeline audioStemClips updated:',
    $audioStemClips.length,
    $audioStemClips.map(c => c.stemType)
  );
    ...
    try {
      ...
      if (voiceSegments.length > 0) {
        trackConfigs.update(configs =>
          configs.map(config =>
            config.type === 'voice-segments' ? { ...config, visible: true } : config
          )
        );
  ...
  async function loadAudioClipsDirectly() {
    try {
      ...
      if (audioStems && audioStems.length > 0) {
        ...
        const latestStems = audioStems.reduce(
          (acc, stem) => {
        ...
        const clips = await Promise.all(
          Object.values(latestStems).map(async (stem, index) => {
  ...
  function getAudioTrackLabel(trackType: string): string {
    ...
    switch (trackType) {
      case 'audio-vocals':
        return isEn ? 'Vocals' : 'Gesang';
      case 'audio-music':
        return isEn ? 'Music' : 'Musik';
      case 'audio-original':
        return isEn ? 'Original' : 'Original';
      default:
        return 'Audio';
    }
  }
  ...
  function getAudioLevelForTrack(trackType: string): number {
    const clip = localAudioClips.find(
      clip =>
  ...
  <div class="tracks-scroll-container" bind:this={scrollContainer}>
    <div class="tracks-inner" style="width: {timelineWidth}px;">
                    ...
                    <div class="track-header">
                      <div class="track-label">
                        <div class="icon-18px text-current">
                          <MaterialSymbol icon={getAudioTrackIcon(track.type)} size={18} />
                        </div>
                      ...
                      <div class="track-controls">
                        <MsqdxButton
                          ...
                          title="Audio Level: {Math.round(
                            getAudioLevelForTrack(track.type) * 100
                          )}%"
      ...
      >
        ...
        {#each Array.from({ length: Math.ceil(timelineDuration / 10) }) as _, j}
  ...
  <div class="timeline-toolbar">
    ...
    <div class="zoom-controls">
      <button
        ...
        title={$currentLocale === 'en' ? 'Zoom Out' : 'Verkleinern'}
      >
        <div class="icon-18px text-current"><MaterialSymbol icon="zoom_out" size={18} /></div>
      </button>
      ...
      <button
        ...
        title={$currentLocale === 'en' ? 'Zoom In' : 'Vergrößern'}
      >
        <div class="icon-18px text-current"><MaterialSymbol icon="zoom_in" size={18} /></div>
      </button>
    </div>
  </div>
</div>
