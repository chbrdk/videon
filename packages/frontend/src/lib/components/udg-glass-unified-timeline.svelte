...
<div class="unified-timeline" bind:this={timelineContainer}>
                  ...
                  <div class="audio-stem-track">
                    ...
                    <div class="track-header">
                      ...
                      <div class="track-controls">
                        <button 
                          class="control-btn small audio-level-btn" 
                          on:click={() => toggleAudioLevelSlider(track.type)} 
                          title="Audio Level: {Math.round(getAudioLevelForTrack(track.type) * 100)}%"
                        >
                          <div class="icon-18px text-current">{@html (getMuteStateForTrack(track.type) ? VolumeOffIcon : VolumeUpIcon)}</div>
                        </button>
  ...
  <div class="timeline-toolbar">
    ...
    <div class="auto-scroll-switch">
      <button 
        class="switch-btn {$autoScroll ? 'active' : ''}"
        on:click={() => {
          const newAutoScroll = !$autoScroll;
          console.log('📜 Auto-Scroll:', $autoScroll, '→', newAutoScroll);
          autoScroll.set(newAutoScroll);
        }}
