<script lang="ts">
  ...
  function parseQwenVLDescription(description: string | null): ParsedQwenVL {
    if (!description) return { sceneInfo: null, summary: null, detailsTitle: 'Detaillierte Beschreibung', details: [] };
    ...
    <div class="ai-description qwen-vl">
      ...
      {:else}
        ...
        <div class="ai-text-raw">
          <p style="white-space: pre-wrap; font-size: 0.875rem; line-height: 1.5; color: rgba(255, 255, 255, 0.9);">
            {qwenVLDescription}
          </p>
        </div>
      {/if}
