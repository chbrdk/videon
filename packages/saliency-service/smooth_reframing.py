    ...
    def reframe_video_smooth(self, video_path: str, saliency_data_path: str, 
        ...
        with tqdm.tqdm(total=total_frames, desc='Smooth reframing') as pbar:
            while True:
                ...
                # **WICHTIG**: Passe die ROI auf die Ziel-Aspect Ratio an
                # Berechne das größtmögliche Rechteck mit Ziel-Aspect Ratio, das in die ROI passt
                crop_center_x = x + w // 2
                crop_center_y = y + h // 2
                
                # Berechne neue Dimensions basierend auf Ziel-Aspect Ratio
                if w / h > target_aspect_ratio[0] / target_aspect_ratio[1]:
                    # ROI ist breiter als Ziel -> passe Breite an
                    new_w = h * target_aspect_ratio[0] // target_aspect_ratio[1]
                    new_h = h
                else:
                    # ROI ist höher als Ziel -> passe Höhe an
                    new_h = w * target_aspect_ratio[1] // target_aspect_ratio[0]
                    new_w = w
                
                # Zentriere das Crop um den ursprünglichen ROI-Mittelpunkt
                new_x = max(0, crop_center_x - new_w // 2)
                new_y = max(0, crop_center_y - new_h // 2)
